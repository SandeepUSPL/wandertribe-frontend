...
<div id="wrap">
    ...
    <div class="row">
        ...
        <div id="content" class="col-md-12">
            ...
            <!-- submenu -->
            <div class="submenu">
                <h1><strong>My Property</strong></h1>
                <div class="collapsed">
                    <a href="#" id="submenutoggle"><h1>My Property<i class="fa fa-bars"></i></h1></a>
                </div>
...
<!-- The Modal -->
<div id="manage_pricing" class="modal">

    <!-- Modal content -->
    <div class="modal-content">
        <div class="modal-header">
            <span class="close" id="close_modal_pricing">&times;</span>
            <h2 class="text-center">Manage Pricing</h2>
        </div>

        <input type="hidden" id="pricingPropertyType">
        <input type="hidden" id="pricingPropertyId">

        <div class="col-lg-12 text-center">
            <button class="btn btn-primary btn-file margin-15" id="new_pricing">Add New Pricing</button>

        </div>
        <div class="modal-body text-center" style="overflow-y: scroll; height:400px;">
            <div class="col-lg-12" id="pricing_common_div">
                ...
                <table class="table table-bordered">
                    ...
                    <tr>
                        ...
                        <th>Pricing</th>
                        <th>Pricing Type</th>
                        <th>Upper Pricing Limit (To ensure GST Slab Rates)</th>
                        <th>Edit</th>
            ...
            <div class="col-lg-12 text-left">
                <h4>Your Base price is:- ₹ <span id="property_base_price" class="big-bold"
                                                 style="color: #1b6d85"></span></h4>
                <h4>Please Note:-</h4>
                <ul>
                    <li>GST Rates for Property booking upto 1000 is NIL</li>
                    <li>GST Rates for Property booking above 1000 to 7500 is 12%</li>
                    <li>GST Rates for Property booking above 7500 is 18%</li>
                </ul>
            </div>
        </div>
...
<script>
    ...
    function list_property() {
        ...
        $.ajax({
            type: "GET",
            url: '<?php echo $_SERVER['API_ENDPOINTS'];?>/api/dashboard/list_property',
            headers: {
                "auth_key": auth_key
            },
            data:{
            ...
            success: function (data, textStatus, xhr) {
                ...
                if (xhr.status == 200) {
                    $.each(data.result, function (index, india) {
                        ...
                        abc += '   <tr> \n ' +
                            '                                        <td>' + number + '</td>\n' +
                            '                                        <td>' + india.camp_name + '</td>\n' +
                            '                                        <td>' + india.camp_type + '</td>\n' +
                            '                                        <td>yes</td>\n' +
                            '                                        <td>' + india.location + '</td>\n' +
                            '                                        <td>' + india.campStatus + '</td>\n' +
                            '                                        <td>\n' +
                            '                                            <button class="btn btn-success btn-file" onclick="edit_property_type(\'' + india.camp_id + '\',\'' + india.camp_code + '\',\'' + india.camp_type + '\');">Edit</button>\n' +
                            '                                        </td> \n' +
                            '                                        <td>\n' +
                            '                                            <button class="btn btn-warning btn-file" onclick="show_manage_inventory(\'' + india.camp_id + '\',\'' + india.camp_code + '\',\'' + india.camp_name + '\');">Manage Inventory</button>\n' +
                            '                                        </td> \n' +
                            '                                        <td>\n' +
                            '                                            <button class="btn btn-warning btn-file" onclick="show_manage_pricing(\'' + india.camp_id + '\',\'' + india.camp_type + '\',\'' + india.price_per_camp + '\');">Manage Pricing</button>\n' +
                            '                                        </td> \n' +
                            '                                        <td>\n' +
                            '                                            <button class="btn btn-cyan btn-file" onclick="add_camp(\'' + india.camp_id + '\');">+ Add</button>\n' +
                            '                                        </td> \n' +
                            '                                    </tr> ';
                        number++;
                    });
    ...
    function show_manage_pricing(id, type, price) {
        $.ajax({
            type: "POST",
            url: '<?php echo $_SERVER['API_ENDPOINTS'];?>/api/get_property_pricing',
            headers: {
                "auth_key": auth_key
            },
            data: {
            ...
            success: function (data, textStatus, xhr) {
                ...
                if (xhr.status == 200) {
                    ...
                    htmlData +=
                        ...
                        '                        <th>Pricing</th>\n' +
                        '                        <th>Pricing Type</th>\n' +
                        '                        <th>Upper Pricing Limit <span class="small">(To ensure GST Slab Rates)</span></th>\n' +
                        '                        <th>Edit</th>\n' +
                        '\n' +
                        '                    </tr>\n' +
                        '                    </thead>\n' +
                        '                    <tbody id="myPricing">\n';
                    $.each(data, function (index, india) {
                        var upLimit = '';
                        if (india.upper_limit == 0) {
                            upLimit = 'No upper limit, GST will be applied accordingly';
                        } else if (india.upper_limit == 1000) {
                            upLimit = 'Upto ₹ 1,000, GST will be NIL';
                        } else if (india.upper_limit == 7500) {
                            upLimit = 'Upto ₹ 7,500, GST will be 12%';
                        } else if (india.upper_limit == 10000) {
                            upLimit = 'Above ₹ 7,500, GST will be 18%';
                        }
                        abc +=
                            '<tr>\n' +
                            '                        <td>' + (index + 1) + '</td>\n' +
                            ...
                            '                        <td>' + india.to_date + '</td>\n' +
                            '                        <td>' + india.pricing_value + '</td>\n' +
                            '                        <td>' + india.pricing_type + '</td>\n' +
                            '                        <td>' + upLimit + '</td>\n' +
                            '                        <td>\n' +
                            '                            <button class="btn btn-cyan btn-file" id="editPriceId" onclick="edit_pricing_list(\'' + india.from_date + '\',\'' + india.to_date + '\',\'' + india.pricing_value + '\',\'' + india.pricing_type + '\',\'' + india.property_id + '\',\'' + india.property_type + '\',\'' + india.upper_limit + '\',\'' + india.id + '\')">Edit\n' +
                            '                            </button>\n' +
                            '                        </td>\n' +
                            '                    </tr>';
                    });
                    finalHtml = htmlData + abc +
                        '                    </tbody>\n' +
                        '                </table>';
                    $("#pricing_common_div").html(finalHtml);
                    $("#property_base_price").html(price);
                    // console.log(finalHtml);
                } else {
                    $("#pricing_common_div").html('No pricing found !! Plz add new pricing');
                }
        ...
        $("#pricingPropertyType").val(type);
        $("#pricingPropertyId").val(id);
...
<script>
    ...
    $("#close_modal_pricing").click(function () {
        $("#manage_pricing").hide();
    });


    function edit_pricing_list(frm, to, price, priceType, id, type, upperLimit, rowId) {

        var html = '<div class="col-md-6 col-md-offset-3 text-left">\n' +
            '        <input type="hidden" id="pricingPropertyType" name="property_type" value="' + type + '">\n' +
            '        <input type="hidden" id="pricingPropertyRow" name="id" value="' + rowId + '">\n' +
            '        <input type="hidden" id="pricingPropertyId" name="property_id" value="' + id + '">\n' +
            '    <div class="form-group">\n' +
            '        <label>From Date:-</label>\n' +
            '        <input type="date" id="pricingFromDate" class="form-control" name="from_date">\n' +
            '    </div>\n' +
            '    <div class="form-group">\n' +
            '        <label>To Date:-</label>\n' +
            '        <input type="date" id="pricingToDate" class="form-control" name="to_date">\n' +
            '    </div>\n' +
            '    <div class="form-group">\n' +
            '        <label>Pricing Percentage:-</label>\n' +
            '        <input type="number" id="pricingValue" max="100" min="0" oninput="validity.valid||(value=\'\');" class="form-control" name="pricing_value">\n' +
            '    </div>\n' +
            '    <div class="form-group">\n' +
            '        <label>Pricing Type:-</label>\n' +
            '        <select name="pricing_type" class="form-control" id="selectPricingType">\n' +
            '            <option value="incremental">Incremental</option>\n' +
            '            <option value="decremental">Decremental</option>\n' +
            '        </select>\n' +
            '    </div>\n' +
            '    <div class="form-group">\n' +
            '        <label>Upper Pricing Limit <span class="small">(To ensure GST Slab Rates)</span>:-</label>\n' +
            '        <select name="upper_limit" class="form-control" id="selectUpperLimit">\n' +
            '            <option value="0">No Limit<span class="small">(GST will be applied accordingly)</span></option>\n' +
            '            <option value="1000">Less Than 1,000<span class="small">(GST:- Nil)</span></option>\n' +
            '            <option value="7500">Less Than 7,500<span class="small">(GST:- 12%)</span></option>\n' +
            '            <option value="10000">Above 7,500<span class="small">(GST:- 18%)</span></option>\n' +
            '        </select>\n' +
            '    </div>\n' +
            '    <div class="form-group text-center">\n' +
            '        <button class="btn btn-amethyst" type="button" onclick="edit_pricing_form()">Submit</button>\n' +
            '        <button class="btn btn-amethyst" type="button" onclick="show_manage_pricing(' + $("#pricingPropertyId").val() + ',\'' + $("#pricingPropertyType").val() + '\')">Go Back</button>\n' +
            '    </div>\n' +
            '</div>';
        $("#pricing_common_div").html(html);

        $("#pricingFromDate").val(frm);
        $("#pricingToDate").val(to);
        $("#pricingValue").val(price);
        $("#selectPricingType").val(priceType);
        $("#selectUpperLimit").val(upperLimit);
    }

    function pricing_limit_verify(value) {
        basePrice = +$("#property_base_price").html();
        if (basePrice < value) {
            var text = 'Your current upper limit is above your base price limit of ₹ ' + basePrice + ' and you have selected a upper limit of ' + value +
    ...
    function edit_pricing_form() {
        if (pricing_limit_verify($("#selectUpperLimit").val())) {
            var endPoint = '<?php echo $_SERVER['API_ENDPOINTS'];?>/api/edit_property_pricing';
            $.ajax({
                type: 'post',
                url: endPoint,
                data: {
                    property_type: $("#pricingPropertyType").val(),
                    id: $("#pricingPropertyRow").val(),
                    property_id: $("#pricingPropertyId").val(),
                    from_date: $("#pricingFromDate").val(),
                    to_date: $("#pricingToDate").val(),
                    pricing_value: $("#pricingValue").val(),
                    pricing_type: $("#selectPricingType").val(),
                    upper_limit: $("#selectUpperLimit").val(),
                },
                dataType: 'json',
                beforeSend: function () {
                    $('.loader-outer').show();
                ...
                success: function (data, textStatus, xhr) {
                    ...
                    if (xhr.status == 200) {
                        show_manage_pricing($("#pricingPropertyId").val(), $("#pricingPropertyType").val());
                    } else {
    ...
    $("#new_pricing").click(function () {
        var html = '<div class="col-md-6 col-md-offset-3 text-left">\n' +
            '    <div class="form-group">\n' +
            '        <label>From Date:-</label>\n' +
            '        <input type="date" class="form-control" name="from_date" id="pricingFromDate">\n' +
            '    </div>\n' +
            '    <div class="form-group">\n' +
            '        <label>To Date:-</label>\n' +
            '        <input type="date" class="form-control" name="to_date" id="pricingToDate">\n' +
            '    </div>\n' +
            '    <div class="form-group">\n' +
            '        <label>Pricing Percentage:-</label>\n' +
            '        <input type="number" id="pricingValue" max="100" min="0" oninput="validity.valid||(value=\'\');" class="form-control" name="pricing_value">\n' +
            ...
            '    <div class="form-group">\n' +
            '        <label>Pricing Type:-</label>\n' +
            '        <select name="pricing_type" id="selectPricingType" class="form-control">\n' +
            '            <option value="incremental">Incremental</option>\n' +
            '            <option value="decremental">Decremental</option>\n' +
            '        </select>\n' +
            '    </div>\n' +
            ...
            '    </div>\n' +
            '    <div class="form-group text-center">\n' +
            '        <button class="btn btn-amethyst" type="button" onclick="new_pricing_form_submit()">Submit</button>\n' +
            '        <button class="btn btn-amethyst" type="button" onclick="show_manage_pricing(' + $("#pricingPropertyId").val() + ',\'' + $("#pricingPropertyType").val() + '\')">Go Back</button>\n' +
            '    </div>\n' +
            '</div>';
        $("#pricing_common_div").html(html);
    ...
    function new_pricing_form_submit() {
        if ($("#pricingFromDate").val() != '' && $("#pricingToDate").val() != '' && $("#pricingValue").val() != '' && $("#selectPricingType").val() != '') {
            if (pricing_limit_verify($("#selectUpperLimit").val())) {
                $.ajax({
                    type: "post",
                    url: "<?php echo $_SERVER['API_ENDPOINTS']; ?>/api/add_property_pricing",
                    data: {
                        from_date: $("#pricingFromDate").val(),
                        to_date: $("#pricingToDate").val(),
                        property_id: $("#pricingPropertyId").val(),
        ...
        $.ajax({
            ...
            success: function (data, textStatus, xhr) {
                if (xhr.status == 200) {
                    ...
                    var total_inventory = data.result.total_inventory;
...
<script src="js/minoral.js"></script><!--Start of Tawk.to Script-->
