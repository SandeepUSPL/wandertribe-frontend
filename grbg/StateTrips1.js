...
$.ajax({
            ...
            <button id=${resp._id} onclick="details(this)">View Details</button>
                    ...
                    if (!(/stay/i).test(resp.provided)) {
                        $('.inclusions>div:nth-of-type(1)').css('display', 'none');
                    }

                    if (!(/food/i).test(resp.provided)) {
                        $('.inclusions>div:nth-of-type(2)').css('display', 'none');
                    }

                    if (resp.vehicleProvided !== "Yes") {
                        $('.inclusions>div:nth-of-type(3)').css('display', 'none');
                    }
                }
                if (r_type === 'SMOOTH HIGHWAY') {
                    $('.future-trips .rt' + i).attr("id", "rt1");
                    }
                    else if (r_type === 'TWIST AND TURN') {
                        $(".future-trips .rt" + i).attr("id", "rt2");
                    } else if (r_type === 'BUMPY RIDES') {
                        $(".future-trips .rt" + i).attr("id", "rt3");
                    } else if (r_type === 'OFFROADING') {
                        $(".future-trips .rt" + i).attr("id", "rt4");
                    }
                    resizeDetail();
                }
            });
