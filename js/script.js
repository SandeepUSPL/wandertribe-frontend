...
//         };
//         //send data to server
//         $.post("https://api.wandertribe.in/contact_us", data, function (s_response) {
...
function get_wallet() {
    ...
    $.ajax({
        type: 'GET',
        url: 'https://api.tripvenza.com/api/wallet_balance',
        crossDomain: true,
        dataType: 'json',
        headers: {
            auth_key: auth_key,
        ...
        success: function (data) {
            var wb = '';
            wb += '<p>₹ ' + data.result.redeemable_balance + '</p>';

            $(".wallet-balance").html(wb);
        },
        statusCode: {
            200: function () {

                $(".wallet-popup").addClass("open");
            },
        },
        error: function (data) {
            console.log('An error occurred.');
...
// 	$.ajax({
// 		type: 'POST',
// 		url: 'http://api.tripvenza.com/api/login',
// 		data: {
...
// 			//this.innerHTML = '<a href="#" class="cws-button border-left large alt gray-dark mb-20">My Account</a>';
// 			this.innerHTML = '<div class="dropdown">' +
// 								'<a href="#" class="cws-button border-left large alt gray-dark mb-20 dropbtn"">My Account</a>' +
// 								'<div class="dropdown-content" id="myDropdown">' +
// 								'<a href="#">Your Wallet</a>  <a href="#">Logout</a>' +
// 							'</div>  </div>' ;
// 			$(".top-login").off('click');
// 		} else {
// 			this.innerHTML = '<a href="#" class="cws-button border-left large alt gray-dark mb-20">LogIn/Sign Up</a>';
// 		}
// 	});
//
// });
...
function forgot_password() {
    sessionStorage.setItem('mobile_verification_type', 'forgot_password');
    top.location.href = 'ForgotPassword.php'
}

function fb_login(endpoint) {
    sessionStorage.setItem('mobile_verification_type', 'social_login');
    top.location.href = endpoint + '/socialMediaRedirect/facebook';
}

function gp_login(endpoint) {
    top.location.href = endpoint + '/socialMediaRedirect/google';
}

function login(endpoint) {
    localStorage.setItem("mobile",document.getElementById('user_mobile').value);
    localStorage.setItem("password",document.getElementById('user_password').value);
    localStorage.setItem("device",document.getElementById('login_device').value);
    $.ajax({
...
function lazyload() {
    ...
    for (var i = 0; i < imgDefer.length; i++) {
        if (imgDefer[i].getAttribute('data-src')) {
            ...
            console.log(imgDefer[i].getAttribute('src'));
...
$(document).ready(function () {
    $("input[name='remember_password']").each(function () {
        $(this).after(' <p class="password-match" style="color: red"></p>');
    })
});


$("input[name='remember_password']").keyup(function(){
    if ($(this).val() != $(this).closest(".checkout").find("input[name='password']").val()){
        $(this).siblings(".password-match").html("Passwords must match");
        $(this).closest(".checkout").find(".btn_register").attr('disabled','disabled');
    }
    else{
        $(this).siblings(".password-match").html("");
        $(this).closest(".checkout").find(".btn_register").removeAttr('disabled');
    }
});

function search_by_category(catg) {
    ...
    today = mm+'/'+dd+'/'+yyyy;
    var tomm = mm+'/'+(dd+1)+'/'+yyyy;

    top.location.href = 'search.php?search.php?destination_name='+catg+'&from_date='+today+'&to_date='+tomm+'&booking_required=2&type="category"';
}

// setInterval(checkAuth, 6000);
// function checkAuth() {
//     var auth_key = localStorage.getItem("auth_key");
//     var device = 'web';
//     if(auth_key){
...
//
//             type: 'POST',
//             url: 'https://api.wandertribe.in/api/check_auth',
//             headers: {
//                 auth_key: auth_key,
//
//             },
