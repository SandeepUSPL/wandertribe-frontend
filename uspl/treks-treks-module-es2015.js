...
let TreksRoutingModule = class TreksRoutingModule {
};
...
let TreksComponent = class TreksComponent {
    constructor() { }
    ngOnInit() {
    }
};
...
let TreksModule = class TreksModule {
};
...
let TrekscheckoutComponent = class TrekscheckoutComponent {
    ...
    ngOnInit() {
        ...
        this.demandStore.select('demand').subscribe(demandState => {
            if (demandState.trekDemand.activeTrek) {
                this.activeTrek = demandState.trekDemand.activeTrek;
            }
            if (demandState.trekDemand.selectedDetails) {
                this.selectedPersonDetails = demandState.trekDemand.selectedDetails;
            }
...
TrekscheckoutComponent.ctorParameters = () => [
...
let TreksdetailsComponent = class TreksdetailsComponent {
    ...
    ngOnInit() {
        this.route.paramMap.subscribe((params) => {
            if (params.get('id')) {
                ...
                this.store.dispatch(new _store_demand_actions__WEBPACK_IMPORTED_MODULE_3__["TryFetchTrek"]({ _id: this.id }));
        ...
        this.store.select('demand').subscribe(demandState => {
            if (demandState.trekDemand.activeTrek) {
                this.activeTrek = demandState.trekDemand.activeTrek;
                this.trekBannerImages = demandState.trekDemand.activeTrek.trekImage;
                if (demandState.trekDemand.activeTrek.similarTreks.length > 0) {
                    demandState.trekDemand.activeTrek.similarTreks.forEach(trek => {
                        this.similarTrek.push(trek);
                    });
                }
                console.log(this.similarTrek);
            }
        });
    }
    adultControl(x) {
    ...
    getPricing() {
        this.http.post(_shared_serverEndpoints__WEBPACK_IMPORTED_MODULE_7__["trekPricing"], this.queryForm.value, { observe: 'response' }).subscribe((response) => {
            if (response.status === 200) {
...
TreksdetailsComponent.ctorParameters = () => [
...
let TrekslistComponent = class TrekslistComponent {
    constructor(store, fb, route, router, http, calendar, formatter) {
        this.store = store;
        this.fb = fb;
        this.route = route;
        this.router = router;
    ...
    ngOnInit() {
        this.http.post(_shared_serverEndpoints__WEBPACK_IMPORTED_MODULE_5__["trekList"], this.requestData, { observe: 'response' }).subscribe((response) => {
            if (response.status === 200) {
                this.store.dispatch(new _store_demand_actions__WEBPACK_IMPORTED_MODULE_6__["SetAllTrek"](response.body));
            }
        });
        ...
        this.no_treks = false;
        this.show_all = false;
        this.trekSubscription = this.store.select('demand').subscribe(demandState => {
            if (demandState.trekDemand.allTreks) {
                this.allTreks = [];
                demandState.trekDemand.allTreks.forEach(trek => {
    ...
    search(form) {
        ...
        this.http.post(_shared_serverEndpoints__WEBPACK_IMPORTED_MODULE_5__["trekQuery"], this.basicForm.value, { observe: 'response' }).subscribe((response) => {
            if (response.status === 200) {
                this.isSearched = false;
                this.reletive_treks = false;
                this.no_treks = false;
    ...
    statewiselist(id, state_name) {
        this.state = state_name;
        this.statewiseTreks = this.allTreks.filter(obj => {
            return obj._id === id;
        });
        this.reletive_treks = false;
        this.no_treks = false;
...
TrekslistComponent.ctorParameters = () => [
    { type: _ngrx_store__WEBPACK_IMPORTED_MODULE_2__["Store"] },
...
let AddtrekComponent = class AddtrekComponent {
    ...
    ngOnInit() {
        ...
        this.store.select('treks').subscribe(trekState => {
            if (!this.editMode) {
                if (trekState.activeTrek) {
                    this.finalTrek = trekState.activeTrek;
                    if (trekState.activeTrek.trekDayDetails) {
                        this.trekDaysArray = trekState.activeTrek.trekDayDetails;
                    }
                }
            }
        });
        ...
        this.route.paramMap.subscribe((params) => {
            if (params.get('id')) {
                ...
                this.id = +params.get('id');
                this.store.select('treks').subscribe(trekState => {
                    this.finalTrek = trekState.treks[this.id];
                    if (this.finalTrek) {
                        this.basicForm.patchValue(this.finalTrek);
                        this.settingForm.patchValue(this.finalTrek);
                        this.finalTrek.trekDayDetails.forEach((value) => {
                            this.trekDayDetails.push(this.addTrekDayDetailGroup(value));
                        });
                        this.trekForm.setValue(this.trekDayDetails.value[0]);
                    }
                    else {
    ...
    addTrekDate() {
        let ngbDate = this.fromDate;
        let fromdate = new Date(ngbDate.year, ngbDate.month - 1, ngbDate.day);
        let ngbDate1 = this.toDate;
        let todate = new Date(ngbDate1.year, ngbDate1.month - 1, ngbDate1.day);
        const x = {
            startDate: fromdate,
            toDate: todate,
        };
    ...
    imgPreviewTrekDay(fileInput) {
        this.trekDayImg = fileInput.target.files[0];
        const mimeType = this.trekDayImg.type;
        ...
        reader.onload = () => {
            this.imgPreviewUrlTrekDay = reader.result;
        };
    }
    imgPreviewTrek(fileInput) {
        ...
        if (files.length >= 1) {
            for (const image of files) {
                ...
                const reader = new FileReader();
                reader.onload = (e) => {
                    this.basicForm.get('trekImage').push(this.fb.group({
                        url: e.target.result
                    }));
                };
    ...
    settingFormSubmit() {
        ...
        if (this.trekImgArray.length) {
            const data = new FormData();
            data.append('_id', this.finalTrek._id);
            Array.from(this.trekImgArray).forEach(image => {
                data.append('trekImage[]', image);
            });
            this.store.dispatch(new _store_treks_actions__WEBPACK_IMPORTED_MODULE_6__["TryUploadTrekImage"](data));
        }
        this.router.navigate(['../manageTrek'], { relativeTo: this.route });
...
AddtrekComponent.ctorParameters = () => [
    { type: _angular_router__WEBPACK_IMPORTED_MODULE_7__["Router"] },
    { type: _angular_forms__WEBPACK_IMPORTED_MODULE_2__["FormBuilder"] },
    { type: _ng_bootstrap_ng_bootstrap__WEBPACK_IMPORTED_MODULE_3__["NgbCalendar"] },
    { type: _ng_bootstrap_ng_bootstrap__WEBPACK_IMPORTED_MODULE_3__["NgbDateParserFormatter"] },
...
let ManagetrekComponent = class ManagetrekComponent {
...
ManagetrekComponent.ctorParameters = () => [
...
const TRY_SAVE_TREK = 'TRY_SAVE_TREK';
const TRY_FETCH_TREKS = 'TRY_FETCH_TREKS';
const TRY_UPLOAD_TREK_IMAGE = 'TRY_UPLOAD_TREK_IMAGE';
const SET_TREKS = 'SET_TREKS';
const SET_ACTIVE_TREK = 'SET_ACTIVE_TREK';
class TryUploadTrekImage {
    constructor(payload) {
        this.payload = payload;
        this.type = TRY_UPLOAD_TREK_IMAGE;
    }
}
TryUploadTrekImage.ctorParameters = () => [
    { type: undefined }
];
class TryFetchTreks {
    constructor() {
...
let TreksEffects = class TreksEffects {
    constructor(action, http) {
        this.action = action;
        this.http = http;
        this.uploadImage = this.action.pipe(Object(_ngrx_effects__WEBPACK_IMPORTED_MODULE_3__["ofType"])(_treks_actions__WEBPACK_IMPORTED_MODULE_5__["TRY_UPLOAD_TREK_IMAGE"]), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_6__["map"])((data) => {
            return data.payload;
        }), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_6__["switchMap"])((data) => {
            return this.http.post(_shared_serverEndpoints__WEBPACK_IMPORTED_MODULE_1__["uploadTrekImage"], data, { observe: 'response' });
        ...
        this.editTrek = this.action.pipe(Object(_ngrx_effects__WEBPACK_IMPORTED_MODULE_3__["ofType"])(_treks_actions__WEBPACK_IMPORTED_MODULE_5__["TRY_SAVE_TREK"]), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_6__["map"])((data) => {
            return data.payload;
        }), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_6__["switchMap"])((data) => {
            return this.http.post(_shared_serverEndpoints__WEBPACK_IMPORTED_MODULE_1__["saveTrek"], data, { observe: 'response' });
        }), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_6__["switchMap"])((response) => {
            if (response.status === 200) {
                return [
                    {
        ...
        this.fetchTreks = this.action.pipe(Object(_ngrx_effects__WEBPACK_IMPORTED_MODULE_3__["ofType"])(_treks_actions__WEBPACK_IMPORTED_MODULE_5__["TRY_FETCH_TREKS"]), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_6__["switchMap"])(() => {
            return this.http.get(_shared_serverEndpoints__WEBPACK_IMPORTED_MODULE_1__["fetchTreks"], { observe: 'response' });
        }), Object(rxjs_operators__WEBPACK_IMPORTED_MODULE_6__["switchMap"])((response) => {
            if (response.status === 200) {
...
function TreksReducer(state = initialState, action) {
    switch (action.type) {
        case 'SET_ACTIVE_TREK': {
            return Object.assign({}, state, { activeTrek: action.payload });
        }
        case 'SET_TREKS': {
            return Object.assign({}, state, { treks: action.payload });
        }
        default: {
...
let TreksBusinessRoutingModule = class TreksBusinessRoutingModule {
};
...
let TreksBusinessModule = class TreksBusinessModule {
};
...
//# sourceMappingURL=treks-treks-module-es2015.js.map